...
# Finds ALL ticket files regardless of prefix (supports Jira-imported tickets)
...
# Find ALL .md ticket files (any prefix), sorted by name descending
TICKET_FILES=$(find "$TICKETS_DIR" -type f -name "*.md" ! -name ".gitkeep" | sort -r)
...
for TICKET_FILE in $TICKET_FILES; do
    ...
    # Extract ticket ID from filename: PREFIX-NUMBER (with optional letter suffix)
    TICKET_ID=$(basename "$TICKET_FILE" | grep -oE '[A-Z]+-[0-9]+[a-z]*')
